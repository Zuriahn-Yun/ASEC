...
async function runSastWorkflow(scanId: string, repoDir: string, enabled: boolean): Promise<number | null> {
  if (!enabled) {
...
async function runDastWorkflow(
  scanId: string,
  repoDir: string,
  detection: Awaited<ReturnType<typeof detectFramework>>,
  enabled: boolean,
): Promise<{ zap: number | null; nuclei: number | null }> {
  if (!enabled) {
...
async function runScaWorkflow(
  scanId: string,
  repoDir: string,
  enabled: boolean,
): Promise<{ trivy: number | null; npmAudit: number | null }> {
  if (!enabled) {
